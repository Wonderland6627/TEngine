...
- [x] 关卡配置拆分合并脚本 便于快速配置某一关
- [ ] 前三关难度简单
- [ ] 后面关卡主要设计成能够多打1

- [x] 在第二关胜利后和排行榜页提醒用户触发CreateUserInfoButton

- [ ] 技能卡系统：通过抽卡获得技能卡（原本的锦囊系统）（Buff向）
- [ ] 技能卡分品质（可升星）
- [ ] 技能改成开始游戏前选择、游戏中手动触发、可以使用金币或广告刷新（刷新必得高级锦囊）
- [ ] 有8个卡槽，可以放置技能卡，然后可以在游戏中随机到这些卡
...
- [x] 获得方式：每日签到、推关奖励
- [ ] 金币作用1：通用技能使用（关卡内）
- [ ] 金币作用2：抽奖（奖励列表：图鉴、技能卡、图鉴碎片、技能卡碎片）
- [ ] 金币作用3：图鉴升级、技能卡升级
- [ ] 根据剩余史莱姆数量决定奖励金币的数值范围
      添加xN倍的进度条 和看广告奖励翻倍

- [x] 通关奖励：返还扣除的体力值
- [x] 额外的金币奖励

- [ ] 图鉴系统：通过抽卡获得史莱姆图鉴（收集向）
https://zh.stardewvalleywiki.com/%E5%8F%B2%E8%8E%B1%E5%A7%86%EF%BC%88%E6%80%AA%E7%89%A9%EF%BC%89
- [ ] 类型图鉴、表情图鉴
- [ ] 扩展：赛尔号基地、PVE花园、鱼缸
...
- [ ] 战斗玩法细化
- [ ] 机制型城堡：打破“纯数值”对撞
- [ ] 加速站（加速城堡）： 经过该城堡发出的路径，史莱姆移动速度提升 $100\%$。占领它意味着你可以发动“闪击战”。
- [ ] 防御塔（箭塔城堡）： 不生产史莱姆，但会射击进入攻击范围的敌方史莱姆。这是防守要塞，也是兵家必争之地。
- [ ] 实验室（变异城堡）： 占领后，从这里派出的史莱姆会随机带有一种特殊属性（如：体积变大、分裂或死亡时产生小范围爆炸）。
- [ ] 金矿（资源城堡）： 不产兵，但持续产生金币。用于局外养成或局内临时升级。

- [ ] 路径策略：从“单线”到“地形”
- [ ] 单向通路： 只能从 A 滑向 B，不能反向。这可以设计出“高地压制”的关卡感。
- [ ] 脆弱路径（冰面）： 路径有承载上限，如果同时在路上的史莱姆过多，路径会断裂（消失 5 秒），强制改变战线。
- [ ] 传送门： 进入 A 城堡的史莱姆会瞬间出现在地图另一端的 B 城堡，这为“偷家”战术提供了可能。

- [ ] 战术深度：兵种克制
- [ ] 基础型 均衡、产出快	游戏前期快速铺场
- [ ] 坦克型 移动慢，1只可抵消敌人2只	强攻高防御建筑
- [ ] 刺客型 极速，但在路径上会被轻易消灭	偷袭后方空虚的城堡

- [ ] 系统扩展
- [ ] 多维进化树：玩家收集关卡掉落的“原液”，可以提升史莱姆的属性（如：移动速度、产出效率、防御力）
- [ ] 史莱姆家园（挂机收益）：仿照模拟经营。玩家推关越多，家园底盘越大，产出（金币/体力）就越高。这不仅给了玩家推图的动力，也提供了离线收益，诱导玩家每天回来领奖。
- [ ] 装饰系统：允许玩家使用关卡星级换取装饰物点缀家园，增加视觉成就感。
- [ ] 成就系统：“第一次占领10座城堡”、“单局生产1000只史莱姆”。提供精美的勋章和称号，在微信好友列表中展示。
- [ ] 图鉴系统：抽奖得到不同史莱姆的不同形态、状态等收藏用。
